...
/**
 * @author Felipe Ros @Usage Retorna o tempo em segundos desde o último acesso ao objeto
 * @since 2.0
 * @version 1.0
 */
public class ObjectIdleTime implements Command, Read, Execute<Long> {
  ...
  public Optional<Long> execute() {
    try (Jedis jedis = RedisLib.getJedis().getResource()) {
      ...
      return Optional.of(jedis.objectIdletime(getKey()));
  ...
  private ObjectIdleTime(Builder builder) {
    this.key = builder.key;
  }

  private Query<ObjectIdleTime> query() {
  ...
  public static class Builder implements Execute<Long> {
    ...
    public Query<ObjectIdleTime> build() {
      return new ObjectIdleTime(this).query();
    }
